...
def load_weights_to_reselect(model_conv, reselect_conv):
    model_conv.load_weights(args.save_dir + '/USPS.h5')
...
def plot_histogram(min_distances):
    ...
    plt.title("Distribution of distances on the USPS dataset")
...
def train(x, y):
    ...
    model = model_conv()
    model.load_weights(args.save_dir + '/USPS.h5')
    kmeans_n_init = 140
    ...
    for ite in range(15):
        ...
        usps = np.expand_dims(x, axis=-1)
        y_one_hot = to_categorical(y, num_classes=10)
        net.fit(datagen.flow(usps, y_one_hot, batch_size=datagen_batch_size), epochs=200)

        # multiple threshold voting
        thresholds = [0.252, 0.755, 1.258, 1.761, 2.264]
        ...
        final_reliable_samples_1 = np.expand_dims(final_reliable_samples, axis=-1)
        final_reliable_samples_labels = to_categorical([assignment[i] for i in reliable_samples_indices])

        from sklearn.utils import shuffle
        trusted_samples_shuffled, trusted_labels_shuffled = shuffle(final_reliable_samples_1,
        ...
        if not os.path.exists("weights/USPS"):
            os.makedirs("weights/USPS")

        current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

        filename = f"weights/USPS/reselect_weight_final_{ds_name}_{current_time}_vote_result.h5"
