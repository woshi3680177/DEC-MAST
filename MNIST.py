...
def load_weights_to_reselect(model_conv, reselect_conv):
    model_conv.load_weights(args.save_dir + '/MNIST.h5')
...
def plot_histogram(min_distances):
    ...
    plt.title("Distribution of distances on the MNIST dataset")
...
def train(x, y):
    ...
    model = model_conv()
    model.load_weights(args.save_dir + '/MNIST.h5')
    kmeans_n_init = 140
    ...
    for ite in range(15):
        ...
        filename = f"weights/MNIST/reselect_weight_final_{ds_name}_{current_time}_vote_result.h5"
