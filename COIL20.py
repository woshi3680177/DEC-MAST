...
def reselect_conv(input_shape):
    ...
    output = layers.Dense(20, activation='softmax', name='predictions')(x)
...
def load_weights_to_reselect(model_conv, reselect_conv):
    model_conv.load_weights(args.save_dir + '/COIL20.h5')
...
def plot_histogram(min_distances):
    ...
    plt.title("Distribution of distances on the COIL20 dataset")
    plt.grid(True, alpha=0.5)
    plt.tick_params(labelsize=48)
    plt.show()

def adaptive_datagen(rotation_range, width_shift_range, height_shift_range):
    num_samples = len(x)
    if num_samples < 1000:
        rotation_range *= 2
        width_shift_range *= 2
...
def train(x, y):
    ...
    model = model_conv()
    model.load_weights(args.save_dir + '/COIL20.h5')
    kmeans_n_init = 140
    ...
    for ite in range(15):
        ...
        datagen = adaptive_datagen(rotation_range, width_shift_range, height_shift_range)
        y_one_hot = to_categorical(y, num_classes=20)
        net.fit(datagen.flow(x, y_one_hot, batch_size=batch_size), epochs=200)

        # multiple threshold voting

        thresholds = [1.625, 4.875, 7.95, 11.35, 14.5, 17.75, 20.75]
        threshold_results = {}
        ...
        if not os.path.exists("weights/COIL20"):
            os.makedirs("weights/COIL20")

        current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

        filename = f"weights/COIL20/reselect_weight_final_{ds_name}_{current_time}_vote_result.h5"
...
if __name__ == '__main__':
    ...
    parser.add_argument('ds_name', default='COIL20')
    parser.add_argument('--save_dir', default='results/IDECT')
    parser.add_argument('--threshold', type=float, default=0.7, help='Threshold for selecting reliable samples')
